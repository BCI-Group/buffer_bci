...
%
%   capFile        -- [str] filename for the channel positions                         ('cap_tmsi_mobita_ErrP.txt')
...
if( isempty(capFile) )
    ...
    end; % cap_tmsi_mobita_ErrP.txt default if not selected
end
...
% Name for the file with training data
dname_errp = 'training_data_ErrP';
% Name for the file with the classifier
...
while ( true )
    ...
    if ( numel(devents)==0 )   % If we have not received any event
        continue;
    elseif ( numel(devents)>1 ) % If we have received a phase event
    ...
    % Decide which phase to start based on the event
    switch lower(phaseToRun);
        ...
        case {'calibrate_errp'};
            ...
            [traindata,traindevents,state]=buffer_waitData(opts.buffhost,...
                opts.buffport,[],'startSet',opts.epochEventType,'exitSet',...
            ...
            % remove data that was colected when the stage was already over
            elementsDeleted = 0;
            i = 1;
            while true % Check all the events
                index = i - elementsDeleted;    % We create a new index because
                                                % we might delete elements
                                                % in the array
                if index==length(traindevents)
                    break
                end
                % Take type of event and value
                type = traindevents(index).type;
                value = traindevents(index).value;
                if strcmp(type, opts.epochEventType) && value < 0 % If the value is negative
                    value = -value - 1; % We added -1 before to recognize these events
                    range = ((index-value):index);   % Delete last 'value' events after an event that indicates end of stage.
        ...
        case {'classify_errp'};
            ...
            if ( ~isequal(clsSubj,subject) || ~exist('clsfr','var') )
                ...
                if ( ~(exist([clsfrfile '.mat'],'file') || exist(clsfrfile,'file')) )
                    ...
                    [fn,pth]=uigetfile('.','Pick Error Related Potential classifier');
                    if ( isequal(fn,0) || isequal(pth,0) )
