...
% Connect Cybathalon and Matlab to receive feedback from game
...
% Setup all the parameters needed before we can start the main loop
...
state=[];
endTesting=false;   % If we have finished calibration
prevStage=[];   % Previous known stage of the robot
prevAction = []; % Previous action
endCont = 0;    % Counter to detect if we have reach the end of the stage
stageCont = 0; % Number of stages played so far
toDelete = 0; % Number of instances to delete from the collected data
                % they were collected after the game ended.
startStage = true; % We just started the stage
...
for si=1:max(100000,nSeq);
    ...
    [devents,state,nevents,nsamples]=buffer_newevents(buffhost,...
        buffport,state,'classifier_im.prediction',[],2000);
    
    trlEndTime=getwTime();
    
    % Inform if we did not receive any prediction
    if( isempty(devents) ) % extract the decision value
        ...
        actStage=step(udpr);
        % If we just started the stage
        if startStage
            % Clear the pile of stages from the game
            % For some reason, Cybathalon starts with 5 items on it
            step(udpr);
            step(udpr);
            step(udpr);
            step(udpr);
        ...
        % If we did not get any new stage, then we are in the same one
        if isempty(actStage)
            actStage = prevStage;
            endCont = endCont + 1;
        else
        ...
        try;
            cybathalon.socket.send(javaObject('java.net.DatagramPacket',...
                uint8([10*cybathalon.player+cybathalon.cmddict(predTgt) 0]),1));
            % We need if sequences because the predTgt tag do not
            % correspont to the number of platform on the Cybathalon game
            if predTgt == 3 
        ...
        if endCont>=wait_end % If we have reach the end of the stage
            sendEvent('stimulus_errp.target', - toDelete-1);    % This extra -1 is to delete this event too
            stageCont = stageCont + 1;
            endCont = 0;
            if stageCont >= numStages % If we have completed enough stages
                endTesting = true;
                ...
                % Save previous stage to a log file of past stages.
                ...
                % Write new stage to the level.csv file
                ...
                % Set variables to default before we start a new stage
                startStage = true;
                toDelete = 0;
            end
        end
...
if ( ishandle(fig) ) % thanks message
    set(txthdl,'string',{'That ends the testing phase.',...
        'Thanks for your patience'}, 'visible', 'on', 'color',[0 1 0]);
    pause(3);
    close(fig)
end
