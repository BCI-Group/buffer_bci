...
if( isempty(capFile) )
    ...
    end; % cap_tmsi_mobita_im.txt default if not selected
end
...
dname_im = 'training_data'; % Name for the file that stores training data
cname_im = 'clsfr'; % Name for the file that stores the classifier
...
while ( true )
    ...
    if ( numel(devents)==0 )  % If we did not get any event
        continue;
    ...
    % Select which phase we were told to run
    switch lower(phaseToRun);
        ...
        case {'calibrate_im'};
            ...
            [traindata,traindevents,state]=buffer_waitData(opts.buffhost,...
                opts.buffport,[],'startSet',opts.epochEventType,...
                'exitSet',{'stimulus_im.calibration' 'end'},...
                'verb',opts.verb,'trlen_ms',opts.trlen_ms,opts.calibrateOpts{:});
            %remove exit event
            mi=matchEvents(traindevents,{'stimulus_im.calibration'},'end');
            traindevents(mi)=[];
            traindata(mi)=[];
            % Save classifier
        ...
        case {'classify_im'};
            try
                ...
                % Save the new classifier if necessary
                [devents] = buffer_newevents(buffhost,buffport,state,'stimulus.save_IM_clsfr',[],1000);
                if ~isempty(devents)
                    clsSubj=subject;
                    fname=[cname_im '_' subject '_' datestr '_new' ];
                    fprintf('Saving classifier to : %s\n',fname);save([fname '.mat'],'clsfr');
                end
            catch
