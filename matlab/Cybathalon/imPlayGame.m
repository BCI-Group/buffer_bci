...
for si=1:max(100000,nSeq);
    ...
    [devents,state,nevents,nsamples]=buffer_newevents(buffhost,buffport,...
        state,'classifier_im.prediction',[],2000);
    
    trlEndTime=getwTime();
    
    % Inform if there was no prediction.
    if( isempty(devents) ) % extract the decision value
        ...
        % Wait for the ErrP classifier to collect data and classify it
        [devents,state,nevents,nsamples]=buffer_newevents(buffhost,...
            buffport,state,'classifier_errp.prediction',[],1500);
        % Inform if no prediction was found
        if( isempty(devents) )
