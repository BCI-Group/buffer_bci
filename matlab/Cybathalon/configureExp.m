...
%% Setup the path and create the connect to the buffer
...
verb         =1; % verbosity level for debug messages, 1=default, 0=quiet
buffhost     ='localhost';
...
nSymbs       =3; % Number of circles in the interface
symbCue      ={'Feet' 'Left-Hand' 'Right-Hand'}; % Name of each bodypart
baselineClass='99 Rest'; % Name for the rest state
nSeq         =15*nSymbs; % 15 examples of each target
nSeq_Prac    =2*nSymbs; % Number of item to practice with in the Practice phase
epochDuration     =1.5; % Amount of time we collect data for imaginary movement
trialDuration     = 3; % Duration of a instruction on screen
...
freqband_im = [7 8 28 29];  % Frequency band to consider for imaginary movement
...
trainOpts_im={'badchrm',1,'width_ms',welch_width_ms,'badtrrm',1,...
    'spatialfilter','trwht','adaptivespatialfilt',trialadaptfactor,...
    'objFn','mlr_cg','binsp',0,'spMx','1vR'}; % adaptive-whiten + direct multi-class training

% ErrP classifier options

freqband_errp = [0.5 1 9.5 10]; % Frequency band to consider for ErrP signal
trainOpts_errp = {'spatialfilter','car','badchrm',1,'badtrrm',1,'overridechnms',1};
