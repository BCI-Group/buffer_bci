function [testdata,testevents]=event_applyClsfrIM(clsfr,varargin)
...
while ( ~endTest )
  ...
  if ( sendPred )
    ...
    to_update = buffer_newevents(buffhost, buffport, [], 'update', [], 1500);
    if ~isempty(to_update)
        if to_update.value
            y = str2num(to_update.value);
            clsfr.update(X_, y);
        end;
    end
  end
